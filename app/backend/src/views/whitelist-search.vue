<template>
    <div class="backend-content" id="content">
        <div class="column col-12 col-xs-12">
          <h3 class="s-title">Rechercher des éléments déjà whitelist</h3>
          <div class="form-group">
            <textarea class="form-input" id="input-example-3" placeholder="Paste the elements here" rows="3" v-model="elements"></textarea>
          </div>
          <div class="form-group">
            <button class="btn btn-primary col-12"  v-on:click="search_elements()">Rechercher</button>
          </div>
          <div class="form-group" v-if="results.length>0 ">
            ...
            <div v-else-if="first_search==false">
                <div class="empty">
                    <p class="empty-title h5">Elément<span v-if="this.elements.match(/[^\r\n]+/g).length>1">s</span> pas trouvé.</p>
